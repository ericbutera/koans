...
class AboutBlocks < Neo::Koan
  def method_with_block
    result = yield # where is the param?
    # the block is the param
    result
  end

  def test_methods_can_take_blocks
    yielded_result = method_with_block { 1 + 2 }
    # this calls method_with_block passing in { 1 + 2 }
    # yield inside references { 1 + 2 }
    assert_equal 3, yielded_result

    yr2 = method_with_block { "moo" + "oink" }
    assert_equal "moooink", yr2
  end
