...
class AboutClassMethods < Neo::Koan
  ...
  def test_objects_have_methods
    fido = Dog.new
    assert fido.methods.size > 0 #67
  end

  def test_classes_have_methods
    assert Dog.methods.size > 0 #122
  end
  ...
  def test_other_objects_are_not_affected_by_these_singleton_methods
    ...
    rover = Dog.new
    def fido.wag
      # add method to instance... interesting
      # i guess this makes sense if everything is an object
      :fidos_wag
    end
  ...
  #
  # I would always choose #1 as #2 is too sure it works but why.
  # Of course, there are always reasons for things. Maybe it's a way of 
  # dynamic composition.
