...
class AboutMethods < Neo::Koan
  ...
  def test_calling_private_methods_without_receiver
    assert_equal "a secret", my_private_method # no receiver; simply `my_private_method`, not `self._my_private_method`
  end

  if before_ruby_version("2.7")   # https://github.com/edgecase/ruby_koans/issues/12
    def test_calling_private_methods_with_an_explicit_receiver
      exception = assert_raise(NoMethodError) do
        self.my_private_method #receiver=self
      end
